...
Algebra(4, 1, () => {
    ...
    var robot = document.body.appendChild(this.graph(() => {
        ...
        var xst_target = abdomen_c; // 4 attacking spots: vertex, abdomen_c
        var Xsc_f = up( xst_target + l_sword * Rsc_if * (-Trans_dir_st) * ~Rsc_if ); // head
        // var Xsc_f = up( xst_target + l_sword * Rsc_if * (-Trans_dir_st) * ~Rsc_if ); // abdomen
        ...
        if (t >= 0 && t < t_swing / 2) {
          ...
          var angs_cubic = nu0II + nu1II * t_tilde + nu2II * (t_tilde**2) + nu3II * (t_tilde**3);
          // var dR0sc = Math.E ** ( -(angs_cubic - ang_Xste1e2) * (Trans_dir_st ^ e3) / 2);
          var tilt_parabola = 0.66 * ((t_tilde - (t_swing / 4) * 1.3) ** 2 - ((t_swing / 4) * 1.3) ** 2);
          var dR0sc = Math.E ** ( -(angs_cubic - ang_Xste1e2) * (Trans_dir_st ^ e3) / 2 - tilt_parabola * (vec_L_X1Xst_proj / I3) / 2);
          
        }   
        var tilt_parabola_f = 0.66 * ((dt - (t_swing / 4) * 1.3) ** 2 - ((t_swing / 4) * 1.3) ** 2);
        var dR0sc_f = Math.E ** ( -(angs_cubic - ang_Xste1e2) * (Trans_dir_st ^ e3) / 2 - tilt_parabola_f * (vec_L_X1Xst_proj / I3) / 2);
        xst_target = abdomen_l;

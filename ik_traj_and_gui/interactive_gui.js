...
Algebra(4, 1, () => {
    ...
    //var robot = document.body.appendChild(this.graph(() => {
      ...
      replaceCanvas(this.graph(()=>{
        //////////////////////////////////////////////////////////////////////
        ////////// Human skeleton and sword dimensions ///////////////////////
        //////////////////////////////////////////////////////////////////////
        
        // Human skeleton points
        var init_x = 0.9;
        var nose = (a3 + d5) * 4 * init_x * e1 + (d4) * 1.5 * e2 + (d1 + a2 - d6) * 3 * 0.83 * e3;var eye_r = (a3 + d5) * 4 * 0.99 * e1 + (d4) * (1.5 + 0.1) * e2 + (d1 + a2 - d6) * 3 * 0.85 * e3;
        var eye_l = (a3 + d5) * 4 * init_x * e1 + (d4) * (1.5 - 0.1) * e2 + (d1 + a2 - d6) * 3 * 0.85 * e3;
        var ear_r = (a3 + d5) * 4 * init_x * e1 + (d4) * (1.5 + 0.4) * e2 + (d1 + a2 - d6) * 3 * 0.84 * e3;      
        var ear_l = (a3 + d5) * 4 * init_x * e1 + (d4) * (1.5 - 0.4) * e2 + (d1 + a2 - d6) * 3 * 0.84 * e3;      
        var shoulder_r = (a3 + d5) * 4 * init_x * e1 + (d4) * (1.5 + 0.9) * e2 + (d1 + a2 - d6) * 3 * 0.65 * e3; 
        var shoulder_l = (a3 + d5) * 4 * init_x * e1 + (d4) * (1.5 - 0.9) * e2 + (d1 + a2 - d6) * 3 * 0.65 * e3; 
        var elbow_r = (a3 + d5) * 4 * init_x * e1 + (d4) * (1.5 + 1.6) * e2 + (d1 + a2 - d6) * 3 * 0.6 * e3;    
        var elbow_l = (a3 + d5) * 4 * init_x * e1 + (d4) * (1.5 - 1.6) * e2 + (d1 + a2 - d6) * 3 * 0.6 * e3;    
        var wrist_r = (a3 + d5) * 4 * init_x * e1 + (d4) * (1.5 + 2.9) * e2 + (d1 + a2 - d6) * 3 * 0.5 * e3;    
        var wrist_l = (a3 + d5) * 4 * init_x * e1 + (d4) * (1.5 - 2.9) * e2 + (d1 + a2 - d6) * 3 * 0.5 * e3;
        var hip_r = (a3 + d5) * 4 * init_x * e1 + (d4) * (1.5 + 1) * e2 + (d1 + a2 - d6) * 3 * 0.4 * e3;       
        var hip_l = (a3 + d5) * 4 * init_x * e1 + (d4) * (1.5 - 1) * e2 + (d1 + a2 - d6) * 3 * 0.4 * e3;       
        var knee_r = (a3 + d5) * 4 * init_x * e1 + (d4) * (1.5 + 1.1) * e2 + (d1 + a2 - d6) * 3 * 0.25 * e3;        
        var knee_l = (a3 + d5) * 4 * init_x * e1 + (d4) * (1.5 - 1.1) * e2 + (d1 + a2 - d6) * 3 * 0.25 * e3;        
        var ancle_r = (a3 + d5) * 4 * init_x * e1 + (d4) * (1.5 + 0.9) * e2 + (d1 + a2 - d6) * 3 * 0.1 * e3;           
        var ancle_l = (a3 + d5) * 4 * init_x * e1 + (d4) * (1.5 - 0.9) * e2 + (d1 + a2 - d6) * 3 * 0.1 * e3;           


        const p = window.state.pose;
        if (p != null && window.state.score > 0.02) {
          nose = p[0][0] * e1 + p[0][1] * e2 + p[0][2] * e3;
          eye_l = p[1][0] * e1 + p[1][1] * e2 + p[1][2] * e3;
          eye_r = p[2][0] * e1 + p[2][1] * e2 + p[2][2] * e3;
          ear_l = p[3][0] * e1 + p[3][1] * e2 + p[3][2] * e3;
          ear_r = p[4][0] * e1 + p[4][1] * e2 + p[4][2] * e3;
          shoulder_l = p[5][0] * e1 + p[5][1] * e2 + p[5][2] * e3;
          shoulder_r = p[6][0] * e1 + p[6][1] * e2 + p[6][2] * e3;
          elbow_l = p[7][0] * e1 + p[7][1] * e2 + p[7][2] * e3;
          elbow_r = p[8][0] * e1 + p[8][1] * e2 + p[8][2] * e3;
          wrist_l = p[9][0] * e1 + p[9][1] * e2 + p[9][2] * e3;
          wrist_r = p[10][0] * e1 + p[10][1] * e2 + p[10][2] * e3;
          hip_l = p[11][0] * e1 + p[11][1] * e2 + p[11][2] * e3;
          hip_r = p[12][0] * e1 + p[12][1] * e2 + p[12][2] * e3;
          knee_l = p[13][0] * e1 + p[13][1] * e2 + p[13][2] * e3;
          knee_r = p[14][0] * e1 + p[14][1] * e2 + p[14][2] * e3;
          ancle_l = p[15][0] * e1 + p[15][1] * e2 + p[15][2] * e3;
          ancle_r = p[16][0] * e1 + p[16][1] * e2 + p[16][2] * e3;     
        } else {
        ...
        //var abdomen_c = (abdomen_r + abdomen_l) / 2;
        
        var throat = shoulder_c + 1.5 * ( (shoulder_r - shoulder_c).Length ) * e3;
        var nose = shoulder_c + 1 * ( (shoulder_r - shoulder_c).Length ) * e3;
        // var throat = ear_c;
        // var vertex = ear_c + 4 * (eye_c - ear_c);
        var vertex = shoulder_c + 2.25 * ( (shoulder_r - shoulder_c).Length ) * e3;
        var abdomen_r = hip_r + 0.3 * (shoulder_r - hip_r);
        var abdomen_l = hip_l + 0.3 * (shoulder_l - hip_l);
        var abdomen_c = (abdomen_r + abdomen_l) / 2;
        ...
        var l_sword = 1090 * mm2m; // sword length
        ...
        var Xst_reset = up(throat);
        // var Xst_reset = up( (a3 + d5) * 4 * 0.99 * e1 + (d4)* 1.5 * e2 + (d1 + a2 - d6) * 3 * 0.83 * e3 );
        // var Xst_reset = up( (a3 + d5) * 6 * 0.99 * e1 + (d4)* 1.5 * e2 + (d1 + a2 - d6) * 3 * 0.83 * e3 ); // unreachable case
        
        var P_noX1st = (no ^ e3 ^ Xst_reset ^ ni).Normalized;
        ...
        var L_X1Xst =  (X1 ^ Xst_reset ^ ni).Normalized;
        var L_X1Xst_proj = ( (L_X1Xst | (X1 ^ e1 ^ e2 ^ ni)) | (X1 ^ e1 ^ e2 ^ ni) ).Normalized;
        var vec_L_X1Xst_proj = (L_X1Xst_proj | (ni ^ no)).Grade(1);
        ...
        var Xsc_i = up( down(X1) + 0.25 * (down(Xst_reset) - down(X1)) );
        var orgsc_i = Xsc_i;
        var T0sc_i = 1 - 0.5 * (down(Xsc_i) ^ ni);
        
        // Sword tip initial position
        var Trans_dir_st = (down(Xst_reset) - down(X1)).Grade(1).Normalized;
        ...
        // 4 attacking spots: vertex, abdomen_c, throat, wrist
        var xst_target = vertex; 
        if (window.api.getStatus() == "Body") {
          xst_target = abdomen_c; 
        }
        if (window.api.getStatus() == "Hand") {
          xst_target = abdomen_c; 
        }
        if (window.api.getStatus() == "Throat") {
          xst_target = throat; 
        }
        
        
        var Xsc_f = up( xst_target + l_sword * Rsc_if * (-Trans_dir_st) * ~Rsc_if );
        ...
        var dotx0 = 0; 
        var dotx2 = 0; 
        var dotx1 = 1.5*(x2 - px0) / t_swing;
        var dx0 = x1 - px0; 
        var dx1 = x2 - x1;
        var sdotx0 = dotx1 + dotx0; 
        var sdotx1 = dotx2 + dotx1;
        ...
        if (window.api.getStatus() == "Track") {
            ...
            zsc_f = [];
            t_tilde = 0;
            Xsc = up( kappa0I + kappa1I * t_tilde + kappa2I * (t_tilde**2) + kappa3I * (t_tilde**3) ).Grade(1);
            angs_cubic = nu0I + nu1I * t_tilde + nu2I * (t_tilde**2) + nu3I * (t_tilde**3);
            dR0sc = Math.E ** ( -(angs_cubic - ang_Xste1e2) * (Trans_dir_st ^ e3) / 2 );
          
        }

        // Head and Hand
        if (window.api.getStatus() == "Head" || window.api.getStatus() == "Hand") {
          // Vertex mode
          if (t >= 0 && t < t_swing / 2) {
            t_tilde = t - 0;
            // Real-time
            ...
            t_tilde = t - t_swing / 2;
            // Real-time
            Xsc = up( kappa0II + kappa1II * t_tilde + kappa2II * (t_tilde**2) + kappa3II * (t_tilde**3) ).Grade(1);
            angs_cubic = nu0II + nu1II * t_tilde + nu2II * (t_tilde**2) + nu3II * (t_tilde**3);
            dR0sc = Math.E ** ( -(angs_cubic - ang_Xste1e2) * (Trans_dir_st ^ e3) / 2 );
        ...
        if (window.api.getStatus() == "Body") {
          ...
          if (t >= 0 && t < t_swing / 2) {
            ...
            Xsc = up( kappa0II + kappa1II * t_tilde + kappa2II * (t_tilde**2) + kappa3II * (t_tilde**3) ).Grade(1);
            angs_cubic = nu0II + nu1II * t_tilde + nu2II * (t_tilde**2) + nu3II * (t_tilde**3);
            tilt_parabola = 0.66 * ((t_tilde - (t_swing / 4) * 1.3) ** 2 - ((t_swing / 4) * 1.3) ** 2);
            dR0sc = Math.E ** ( -(angs_cubic - ang_Xste1e2) * (Trans_dir_st ^ e3) / 2 - tilt_parabola * (vec_L_X1Xst_proj / I3) / 2);
            
          }   
          
          tilt_parabola_f = 0.66 * ((dt - (t_swing / 4) * 1.3) ** 2 - ((t_swing / 4) * 1.3) ** 2);
          dR0sc_f = Math.E ** ( -(angs_cubic - ang_Xste1e2) * (Trans_dir_st ^ e3) / 2 - tilt_parabola_f * (vec_L_X1Xst_proj / I3) / 2);
          xst_target = abdomen_r;
          Xsc_f = up( xst_target + l_sword * Rsc_if * (-Trans_dir_st) * ~Rsc_if );


          // Middle point 2
          tilt_parabola_m2 = 0.66 * (((0.5 * dt) - (t_swing / 4) * 1.3) ** 2 - ((t_swing / 4) * 1.3) ** 2);
          dR0sc_m2 = Math.E ** ( -(angs_cubic - ang_Xste1e2) * (Trans_dir_st ^ e3) / 2 - tilt_parabola_m2 * (vec_L_X1Xst_proj / I3) / 2);
          xst_m2 = down(Xst_i) * 0.25 + xst_target * 0.75;
          Xsc_m2 = up( xst_m2 + l_sword * Rsc_if * (-Trans_dir_st) * ~Rsc_if );
        
          // Middle point 2 orientation
          var R0sc_m2 = R0sc_i * dR0sc_m2;
          var xsc_m2 = (dR0sc_m2 * xsc_i * ~dR0sc_m2).Grade(1);
          var ysc_m2 = (dR0sc_m2 * ysc_i * ~dR0sc_m2).Grade(1);
          var zsc_m2 = (dR0sc_m2 * zsc_i * ~dR0sc_m2).Grade(1);
          // var xsc_m2 = (R0sc_m2 * e1 * ~R0sc_m2).Grade(1);
          // var ysc_m2 = (R0sc_m2 * e2 * ~R0sc_m2).Grade(1);
          // var zsc_m2 = (R0sc_m2 * e3 * ~R0sc_m2).Grade(1); // (BUT NOT) same result as "var zsc_m2 = dR0sc_m2 * zsc_i * ~dR0sc_m2;"
          
          // Rotation matrix to RzRyRx
          var Rot0sc_m2 = [[xsc_m2 | e1, ysc_m2 | e1, zsc_m2 | e1], 
                           [xsc_m2 | e2, ysc_m2 | e2, zsc_m2 | e2], 
                           [xsc_m2 | e3, ysc_m2 | e3, zsc_m2 | e3]];
          var Ry_m2 = Math.atan2(-Rot0sc_m2[2][0], Math.sqrt(Rot0sc_m2[0][0]**2 + Rot0sc_m2[1][0]**2));
          var Rz_m2 = Math.atan2(Rot0sc_m2[1][0] / Math.cos(Ry_m2), Rot0sc_m2[0][0] / Math.cos(Ry_m2));
          var Rx_m2 = Math.atan2(Rot0sc_m2[2][1] / Math.cos(Ry_m2), Rot0sc_m2[2][2] / Math.cos(Ry_m2));
          var R0sc_m2z = Math.E ** ( -Rz_m2 * (e1 ^ e2) / 2 );
          var R0sc_m2y = Math.E ** ( -Ry_m2 * (e3 ^ e1) / 2 );
          var R0sc_m2x = Math.E ** ( -Rx_m2 * (e2 ^ e3) / 2 );
          var R0sc_m2test =  R0sc_m2z * R0sc_m2y * R0sc_m2x;
          var xsc_m2test = (R0sc_m2test * e1 * ~R0sc_m2test).Grade(1);
          var ysc_m2test = (R0sc_m2test * e2 * ~R0sc_m2test).Grade(1);
          var zsc_m2test = (R0sc_m2test * e3 * ~R0sc_m2test).Grade(1); // the same x-y-z- axes with the use of rotors
        ...
        var R0scz_cmd_m2;
        if (window.api.getStatus() == "Body") {
          xsc_cmd_m2 = (down(Xsc_m2) | e1)[0] * m2mm;
          ysc_cmd_m2 = (down(Xsc_m2) | e2)[0] * m2mm;
          zsc_cmd_m2 = (down(Xsc_m2) | e3)[0] * m2mm;
          R0scx_cmd_m2 = Rx_m2 * r2d;
          R0scy_cmd_m2 = Ry_m2 * r2d;
          R0scz_cmd_m2 = Rz_m2 * r2d;
        }
      ...
      //console.log("start IK")
        ...
        if (window.api.getStatus() == "Body") {
          ...
          if (window.api.getStatus() === "Track") {
            // 60 / 30 = 2 Hz
            window.api.sendWebSocket(JSON.stringify(pose_i), 25);
          }
          if (window.api.hasTriggered("head") || window.api.hasTriggered("hand")) {
            console.log(window.api.getStatus());
            window.api.sendWebSocket(JSON.stringify(pose_i), 1);
            window.api.sendWebSocket(JSON.stringify(pose_m), 1);
            window.api.sendWebSocket(JSON.stringify(pose_f), 1);
            window.api.sendWebSocket(JSON.stringify(pose_i), 1);
          }
          if (window.api.hasTriggered("throat")) {
            console.log(window.api.getStatus());
            window.api.sendWebSocket(JSON.stringify(pose_i), 1);
            window.api.sendWebSocket(JSON.stringify(pose_f), 1);
            window.api.sendWebSocket(JSON.stringify(pose_i), 1);
          }
          if (window.api.hasTriggered("body")) {
            console.log(window.api.getStatus());
            window.api.sendWebSocket(JSON.stringify(pose_i), 1);
            window.api.sendWebSocket(JSON.stringify(pose_m), 1);
            window.api.sendWebSocket(JSON.stringify(pose_m2), 1);
            window.api.sendWebSocket(JSON.stringify(pose_f), 1);
            window.api.sendWebSocket(JSON.stringify(pose_i), 1);
          }
...
//console.log("b")
        ...
        // Shapes and texts
        pointRadius: 0.75, // custon point radius (default = 1)
